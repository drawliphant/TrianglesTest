...
{
    char t;
    for(int i=0;i<triangleStr.length()-1;i++){
        char t=triangleStr[i];
        if(t>='+'&&t<='9'||t=='|'){
        }else{
            throw std::invalid_argument ("Invalid String input for Triangle Constuctor");
        }
    }
    ...
    if (split(triangleStr, '|', values, 3))
    {
        ...
        m_points[2] = new Point(values[2]);//fix
        setupEdges();
    }
}
...
char Triangle::getTriangleType() const
{
    ...
    if (isValid())
    {
        if (isTriangle())//Take off !
        {
            ...
            if (approximatelyEquals(a, b, m_edgeLengthThreshold)  &&
            ...
            // If any two sides are the same, then its an isosceles
                    ...
                    approximatelyEquals(b, c, m_edgeLengthThreshold) ||
                    approximatelyEquals(c, a, m_edgeLengthThreshold))//c to a not c to c
            {
                result = 'I';
            }
...
double Triangle::computerArea() const
{
    ...
    if (isTriangle())
    {
        ...
        double b = m_edges[1]->getLength();
        double c = m_edges[2]->getLength();
        double s = ( a + b + c)/2;//changed b to c
