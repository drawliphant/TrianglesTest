...
double Edge::getLength() const
{
    ...
    if (isValid())
    {
        ...
        double sumOfSquares = std::pow(diffX,double(2)) + std::pow(diffY,double(2)) + std::pow(diffZ,double(2));
...
double Edge::getSlopeX() const
{
    ...
    if (isValid() && !m_point1->isEquivalentTo(*m_point2))
    {
        double diffY = m_point2->getY() - m_point1->getY();
        double diffZ = m_point2->getZ() - m_point1->getZ();
        double sumOfSquares = std::pow(diffY, double(2)) + std::pow(diffZ, double(2));
...
double Edge::getSlopeY() const
{
    ...
    if (isValid() && !m_point1->isEquivalentTo(*m_point2))
    {
        double diffX = m_point2->getX() - m_point1->getX();
        double diffZ = m_point2->getZ() - m_point1->getZ();
        double sumOfSquares = std::pow(diffX,double(2)) + std::pow(diffZ, double(2));
...
double Edge::getSlopeZ() const
{
    ...
    if (isValid() && !m_point1->isEquivalentTo(*m_point2))
    {
        double diffX = m_point2->getX() - m_point1->getX();
        double diffY = m_point2->getY() - m_point1->getY();
        double sumOfSquares = std::pow(diffX, double(2)) + std::pow(diffY, double(2));
        double xyOffset = std::sqrt(sumOfSquares);

        if (xyOffset == 0)//fix from != to ==
        {
            result = INFINITY;
        }
        else
...
bool Edge::isParallelTo(const Edge &otherEdge)
{
    return isValid() &&
           ...
           areSlopesEquivalent(getSlopeZ(), otherEdge.getSlopeZ());//slope z and other slope x to slope z and other slope z
}
